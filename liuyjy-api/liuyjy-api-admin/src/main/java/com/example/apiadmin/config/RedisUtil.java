...
public class RedisUtil {
    ...
    public boolean test() {
        cacheValue("redisKey", "saddtest");
        getValue("redisKey");

        redisTemplate.opsForSet().add("redisKey1", "saddtest2");
        return false;
    }
   ...
   /* public boolean containsValueKey(String k) {
     ...
     * @return
     */
    public Long increment(String k, Long shu) {
        try {
            ValueOperations<String, String> valueOps = redisTemplate.opsForValue();
            return valueOps.increment(k, shu);
        } catch (Throwable t) {
     ...
     * @return set状态
     */
    public boolean timeSet(String k, String v, TimeUnit unit) {
        String key = k;
        try {
            ValueOperations<String, String> valueOps = redisTemplate.opsForValue();
            valueOps.set(key, v, 5, unit);
            return true;
        } catch (Throwable t) {
            log.error("缓存[" + key + "]失败, value[" + v + "]", t);
        }
